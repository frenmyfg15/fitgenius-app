...
function Shimmer({ children, radius = 12, height }: { children: React.ReactNode; radius?: number; height: number }) {
  const { colorScheme } = useColorScheme();
  const isDark = colorScheme === "dark";
  const t = useShimmer();

  const base = isDark ? Colors.primary : "rgba(15,23,42,0.06)";
...
export default function MisRutinasSkeleton() {
  ...
  return (
    <View
      style={{
        backgroundColor: isDark ? Colors.primary : "#F8FAFC",
        padding: 5,
        paddingBottom: 80,
        minHeight: "100%",
        width: "100%",
