...
export default function ExperienciaModal({ visible, onClose, value = 0 }: Props) {
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      ...
      <View style={styles.center}>
        <View style={[styles.card, { backgroundColor: C.bg, borderColor: C.border }]}>
          <Text style={[styles.title, { color: C.fg }]}>Puntos de experiencia</Text>
          <Text style={[styles.value, { color: C.fg }]}>{new Intl.NumberFormat("es-ES").format(value)}</Text>
          <Text style={[styles.desc, { color: C.mut }]}>
            Ganas XP al completar ejercicios y días de entrenamiento. Mantén consistencia para subir de nivel.
          </Text>
