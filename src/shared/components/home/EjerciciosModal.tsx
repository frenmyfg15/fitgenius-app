...
export default function EjerciciosModal({ visible, onClose, value = 0 }: Props) {
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      ...
      <View style={styles.center}>
        <View style={[styles.card, { backgroundColor: C.bg, borderColor: C.border }]}>
          <Text style={[styles.title, { color: C.fg }]}>Ejercicios totales hoy</Text>
          <Text style={[styles.value, { color: C.fg }]}>{new Intl.NumberFormat("es-ES").format(value)}</Text>
          <Text style={[styles.desc, { color: C.mut }]}>
            Cuenta los ejercicios planificados/realizados en tu sesión de hoy. Mantén un volumen semanal equilibrado.
          </Text>
